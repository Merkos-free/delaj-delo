<purpose>
Показать полный справочник команд ДелайДело. Выведи ТОЛЬКО справочный контент. НЕ добавляй анализ проекта, статус git, предложения следующих шагов или любые комментарии за пределами справочника.
</purpose>

<reference>
# Справочник команд ДелайДело

**ДелайДело** (ДД) создаёт иерархические планы проектов, оптимизированные для соло-агентной разработки с Claude Code.
...
ДелайДело быстро развивается. Обновляйтесь периодически:
...
**`/gsd:new-project`**
Инициализировать новый проект через единый поток.

Одна команда проведёт вас от идеи до готовности к планированию:
...
- Определение требований с разграничением v1/v2/за пределами
...
- `config.json` — режим работы (интерактивный/YOLO)
- `research/` — исследование предметной области (если выбрано)
- `REQUIREMENTS.md` — требования с REQ-ID
...
**`/gsd:map-codebase`**
Картировать существующую кодовую базу для браунфилд-проектов.
...
- Обнаруживает стандартный стек, паттерны архитектуры, подводные камни
...
- Выходит за рамки «какую библиотеку» к знаниям экосистемы
...
- Разбивает фазу на конкретные, выполнимые задачи
- Включает критерии проверки и меры успеха
...
- Группирует планы по волнам (из метаданных), выполняет волны последовательно
...
**`/gsd:quick`**
Выполнить небольшую задачу с гарантиями ДД, пропуская опциональных агентов.
...
Показать список заметок и выбрать для работы.
...
Проверить выполненную работу через диалоговое приёмочное тестирование.
...
├── config.json           # Режим работы и гейты
├── todos/                # Записанные идеи и задачи
│   ├── pending/          # Ожидающие заметки
...
- Подтверждает каждое важное решение
- Останавливается на контрольных точках для утверждения
- Больше руководства на протяжении всего процесса

**Режим YOLO**

- Автоматически утверждает большинство решений
- Выполняет планы без подтверждения
- Останавливается только на критических контрольных точках

Измените в любое время, отредактировав `.planning/config.json`
...
- `false`: Артефакты планирования остаются только локально, не коммитятся

Когда `commit_docs: false`:
- Добавьте `.planning/` в ваш `.gitignore`
- Полезно для вкладов в OSS, клиентских проектов, или для приватности планирования
- Все файлы планирования работают нормально, просто не отслеживаются в git

**`planning.search_gitignored`** (по умолчанию: `false`)
- `true`: Добавить `--no-ignore` к широким ripgrep-поискам
- Нужно только когда `.planning/` в gitignore и вы хотите чтобы поиски по проекту включали его
...
/clear
/gsd:debug                                # Возобновить с того места
```
